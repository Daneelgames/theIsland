...
namespace StylizedWater2
{
    public static class UnderwaterLighting
    {
        ...
        private static int unity_SpecCube0 = Shader.PropertyToID("unity_SpecCube0");
        private static int unity_SpecCube0_HDR = Shader.PropertyToID("unity_SpecCube0_HDR");
        private static int unity_WorldToLight = Shader.PropertyToID("unity_WorldToLight");

        public static void PassAmbientLighting(ScriptableRenderPass pass, CommandBuffer cmd, Material material)
        {
            if (RenderSettings.ambientMode == AmbientMode.Skybox)
            {
                //Only set up on a per-renderer basis, emulate the behaviour for post-processing passes
                cmd.SetGlobalTexture(unity_SpecCube0, ReflectionProbe.defaultTexture);
                cmd.SetGlobalVector(unity_SpecCube0_HDR, ReflectionProbe.defaultTextureHDRDecodeValues);
            }
            
            material.SetVector(_AmbientParams, new Vector4(Mathf.GammaToLinearSpace(RenderSettings.ambientIntensity), RenderSettings.ambientMode == AmbientMode.Skybox ? 1 : 0, 0, 0));
            //URP uses spherical harmonics to store the ambient light color, even if it's flat. But this is done in native engine code
            material.SetColor(_AmbientColor, RenderSettings.ambientLight.linear);
        }
        ...
        public static void PassMainLight(CommandBuffer cmd, RenderingData renderingData)
        {
            ...
            if (renderingData.lightData.mainLightIndex > -1)
            {
                ...
                if (mainLight.lightType == LightType.Directional)
                {
                    ...
                    //finalColor already returns color, but doesn't factor in intensity
                    //For normal renderers, the color is black if the light is disabled
        ...
        {
            public static RenderTextureDescriptor depthNormalDsc;
