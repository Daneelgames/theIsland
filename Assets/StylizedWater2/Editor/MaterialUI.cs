    ...
    public class MaterialUI : ShaderGUI
    {
        ...
        private void DrawLighting()
        {
            ...
            if (EditorGUILayout.BeginFadeGroup(lightingSection.anim.faded))
            {
                ...
                enableLighting = UI.Material.Toggle(enableLighting, "Enable lighting", "Color from lights and Ambient light will affect the material. Can be disabled if using Unlit shaders, or fixed lighting");
                ...
                UI.DrawNotification(enableFlatShading && tesselationEnabled, "Tessellation is enabled, it should not be used to achieve the desired effect", MessageType.Warning);

                UI.DrawNotification(enableFlatShading && _WavesOn.floatValue == 0f, "Flat shading has little effect if waves are disabled", MessageType.Warning);
                ...
                if (enableTranslucency)
                {
                    ...
                    _TranslucencyParams.vectorValue = new Vector4(translucencyStr, translucencyExp, translucencyMask, 0f);
                    EditorGUI.indentLevel--;
                }
